...
)
{
  ...
  if (idx != (uint32_t)0U)
    LowStar_RVector_free_elems___uint8_t__uint32_t(rg, rv, idx - (uint32_t)1U);
}
...
)
{
  if (!(rv.sz == (uint32_t)0U))
    LowStar_RVector_free_elems___uint8_t__uint32_t(rg, rv, rv.sz - (uint32_t)1U);
  free___uint8_t_(rv);
}
...
void mt_sha256_compress(uint8_t *src1, uint8_t *src2, uint8_t *dst)
{
  ...
  EverCrypt_Hash_update_multi(st, (uint64_t)0U, cb, (uint32_t)64U);
...
)
{
  if (!(cidx == (uint32_t)0U))
  {
    ...
    LowStar_RVector_alloc___LowStar_Vector_vector_str__uint8_t__uint32_t(rg,
      rv,
      cidx - (uint32_t)1U);
  }
}
...
)
{
  if (!(cidx == (uint32_t)0U))
  {
    ...
    LowStar_RVector_alloc____uint8_t__uint32_t(rg, rv, cidx - (uint32_t)1U);
  }
}
...
)
{
  ...
  rg.r_free(rg.state, uu____0);
  if (idx != (uint32_t)0U)
    LowStar_RVector_free_elems__LowStar_Vector_vector_str__uint8_t__uint32_t(rg,
      rv,
      idx - (uint32_t)1U);
}
...
)
{
  ...
  if (j % (uint32_t)2U == (uint32_t)1U)
  {
    ...
    MerkleTree_Low_insert_(hsz, lv + (uint32_t)1U, j / (uint32_t)2U, hs, acc, hash_fun);
  }
}
...
void MerkleTree_Low_mt_insert(MerkleTree_Low_merkle_tree *mt, uint8_t *v)
{
  ...
  MerkleTree_Low_insert_(hsz1, (uint32_t)0U, mtv.j, hs, v, mtv.hash_fun);
  *mt
  =
    (
      (MerkleTree_Low_merkle_tree){
...
)
{
  if (!(j == (uint32_t)0U))
  {
    ...
    if (j % (uint32_t)2U == (uint32_t)0U)
      MerkleTree_Low_construct_rhs(hsz,
        lv + (uint32_t)1U,
        hs,
        rhs,
        i / (uint32_t)2U,
...
void MerkleTree_Low_mt_get_root(const MerkleTree_Low_merkle_tree *mt, uint8_t *rt)
{
  ...
  {
    MerkleTree_Low_construct_rhs(hash_size, (uint32_t)0U, hs, rhs, i, j, rt, false, hash_fun);
...
static uint32_t mt_path_length_step(uint32_t k, uint32_t j, bool actd)
{
  if (j == (uint32_t)0U)
    return (uint32_t)0U;
  else if (k % (uint32_t)2U == (uint32_t)0U)
  ...
  else
  {
    ...
    return
      mt_path_length_step(k,
        j,
        actd)
      + MerkleTree_Low_mt_path_length(lv + (uint32_t)1U, k / (uint32_t)2U, j / (uint32_t)2U, nactd);
  }
}
...
)
{
  ...
  if (!(j == (uint32_t)0U))
  {
    ...
    if (j % (uint32_t)2U == (uint32_t)0U)
      ...
      ite = true;
    MerkleTree_Low_mt_get_path_(hsz,
      lv + (uint32_t)1U,
      hs,
      rhs,
      i / (uint32_t)2U,
...
)
{
  ...
  *p = ((MerkleTree_Low_path){ .hash_size = hsz1, .hashes = ipv });
  MerkleTree_Low_mt_get_path_(mtv.hash_size, (uint32_t)0U, hs, rhs, i, j, idx1, p, false);
  return j;
}
...
)
{
  ...
  if (!(oi == opi))
  {
    ...
    MerkleTree_Low_mt_flush_to_(hsz, lv + (uint32_t)1U, hs, pi / (uint32_t)2U, i / (uint32_t)2U);
  }
}
...
void MerkleTree_Low_mt_flush_to(MerkleTree_Low_merkle_tree *mt, uint64_t idx)
{
  ...
  MerkleTree_Low_Datastructures_hash_vv hs = mtv.hs;
  MerkleTree_Low_mt_flush_to_(hsz, (uint32_t)0U, hs, mtv.i, idx1);
  *mt
  =
    (
      (MerkleTree_Low_merkle_tree){
...
)
{
  ...
  if (idx + (uint32_t)1U < rv.sz)
    LowStar_RVector_free_elems_from___uint8_t__uint32_t(rg, rv, idx + (uint32_t)1U);
}
...
)
{
  if (!(lv >= hs.sz))
  {
    ...
    if (lv + (uint32_t)1U < hs.sz)
      MerkleTree_Low_mt_retract_to_(hsz,
        hs,
        lv + (uint32_t)1U,
        i / (uint32_t)2U,
        s / (uint32_t)2U,
...
void MerkleTree_Low_mt_retract_to(MerkleTree_Low_merkle_tree *mt, uint64_t r)
{
  ...
  MerkleTree_Low_Datastructures_hash_vv hs = mtv.hs;
  MerkleTree_Low_mt_retract_to_(mtv.hash_size,
    hs,
    (uint32_t)0U,
    mtv.i,
    r1 + (uint32_t)1U,
  ...
  return
    ...
    &&
      ...
      ==
        (uint32_t)1U
        +
          MerkleTree_Low_mt_path_length((uint32_t)0U,
...
)
{
  ...
  MerkleTree_Low_mt_verify_(hash_size, k1, j1, p, (uint32_t)1U, ih, false, mtv.hash_fun);
...
{
  K___bool_uint32_t scrut = serialize_uint8_t(ok, (uint8_t)(x >> (uint32_t)8U), buf, sz, pos);
  bool ok1 = scrut.fst;
...
{
  K___bool_uint32_t scrut = serialize_uint16_t(ok, (uint16_t)(x >> (uint32_t)16U), buf, sz, pos);
  bool ok1 = scrut.fst;
  uint32_t pos1 = scrut.snd;
...
{
  K___bool_uint32_t scrut = serialize_uint32_t(ok, (uint32_t)(x >> (uint32_t)32U), buf, sz, pos);
  bool ok1 = scrut.fst;
  uint32_t pos1 = scrut.snd;
...
)
{
  if (!ok || pos >= sz)
    ...
    return
      MerkleTree_Low_Serialization_serialize_hash_i(hash_size,
        ok,
        x,
        buf,
        sz,
...
)
{
  ...
  else
  {
    ...
    uint64_t rest = MerkleTree_Low_Serialization_hash_vv_bytes_i(hash_size, vv, i + (uint32_t)1U);
...
)
{
  ...
  else
  {
    ...
    if (x.sz > (uint32_t)0U)
      return
        MerkleTree_Low_Serialization_serialize_hash_vv_i(hash_size,
          ok1,
          x,
          buf,
          sz,
...
uint64_t MerkleTree_Low_Serialization_mt_serialize_size(const MerkleTree_Low_merkle_tree *mt)
{
  ...
  MerkleTree_Low_Datastructures_hash_vv hs = mtv.hs;
  MerkleTree_Low_Datastructures_hash_vec rhs = mtv.rhs;
  uint64_t hs_sz = MerkleTree_Low_Serialization_hash_vv_bytes_i(mtv.hash_size, hs, (uint32_t)0U);
...
)
{
  ...
  MerkleTree_Low_merkle_tree mtv = *mt1;
  K___bool_uint32_t scrut = serialize_uint8_t(true, (uint8_t)1U, output, sz1, (uint32_t)0U);
  bool ok = scrut.fst;
...
)
{
  ...
  uint32_t sz1 = (uint32_t)sz;
  MerkleTree_Low_path *ncp = (MerkleTree_Low_path *)p;
  K___bool_uint32_t scrut = serialize_uint32_t(true, hsz1, output, sz1, (uint32_t)0U);
  bool ok = scrut.fst;
  uint32_t pos = scrut.snd;
  MerkleTree_Low_path ncpd = *ncp;
