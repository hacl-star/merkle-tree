...
alloc_reserve___uint8_t_(uint32_t len, uint8_t *ia)
{
  ...
  for (uint32_t _i = 0U; _i < len; ++_i)
    buf[_i] = ia;
  return ((MerkleTree_Low_Datastructures_hash_vec){ .sz = 0U, .cap = len, .vs = buf });
}

static MerkleTree_Low_Datastructures_hash_vec hash_vec_r_alloc(uint32_t hsz)
{
  KRML_MAYBE_UNUSED_VAR(hsz);
  return alloc_reserve___uint8_t_(1U, NULL);
}
...
{
  rg.r_free(rg.state, index___uint8_t_(rv, idx));
  if (idx != 0U)
    LowStar_RVector_free_elems___uint8_t__uint32_t(rg, rv, idx - 1U);
}
...
{
  if (!(rv.sz == 0U))
    LowStar_RVector_free_elems___uint8_t__uint32_t(rg, rv, rv.sz - 1U);
  free___uint8_t_(rv);
}
...
insert___uint8_t_(MerkleTree_Low_Datastructures_hash_vec vec, uint8_t *v)
{
  ...
  if (sz == cap)
  {
    ...
    nvs[sz] = v;
    KRML_HOST_FREE(vs);
    return ((MerkleTree_Low_Datastructures_hash_vec){ .sz = sz + 1U, .cap = ncap, .vs = nvs });
  }
  else
  {
    vs[sz] = v;
    return ((MerkleTree_Low_Datastructures_hash_vec){ .sz = sz + 1U, .cap = cap, .vs = vs });
  }
}
...
{
  MerkleTree_Low_path x0 = ((MerkleTree_Low_path *)path1)[0U];
  KRML_HOST_IGNORE(x0.hash_size);
...
{
  MerkleTree_Low_merkle_tree x0 = mt[0U];
  KRML_HOST_IGNORE(x0.hash_size);
...
{
  MerkleTree_Low_merkle_tree x0 = ((MerkleTree_Low_merkle_tree *)mt)[0U];
  KRML_HOST_IGNORE(x0.hash_size);
...
{
  MerkleTree_Low_merkle_tree dmt = ((MerkleTree_Low_merkle_tree *)mt)[0U];
  KRML_HOST_IGNORE(dmt.hash_size);
...
void (*MerkleTree_Init_init)(void) = EverCrypt_AutoConfig2_init;
...
{
  return MerkleTree_Low_mt_create_custom(32U, init, mt_sha256_compress);
}

uint32_t MerkleTree_Low_uint32_32_max = 4294967295U;

uint64_t MerkleTree_Low_uint32_max = 4294967295ULL;

uint64_t MerkleTree_Low_uint64_max = 18446744073709551615ULL;

uint64_t MerkleTree_Low_offset_range_limit = 4294967295ULL;

uint32_t MerkleTree_Low_merkle_tree_size_lg = 32U;
...
{
  KRML_MAYBE_UNUSED_VAR(rhs_ok);
  KRML_MAYBE_UNUSED_VAR(mroot);
  return
    j
    >= i
...
)
{
  if (!(cidx == 0U))
  {
    ...
    assign__LowStar_Vector_vector_str__uint8_t_(rv, cidx - 1U, v);
    LowStar_RVector_alloc___LowStar_Vector_vector_str__uint8_t__uint32_t(rg, rv, cidx - 1U);
  }
}
...
)
{
  ...
  LowStar_RVector_alloc___LowStar_Vector_vector_str__uint8_t__uint32_t(rg, vec, len);
  return vec;
}
...
)
{
  if (!(cidx == 0U))
  {
    ...
    assign___uint8_t_(rv, cidx - 1U, v);
    LowStar_RVector_alloc____uint8_t__uint32_t(rg, rv, cidx - 1U);
  }
}
...
alloc_rid___uint8_t__uint32_t(LowStar_Regional_regional__uint32_t__uint8_t_ rg, uint32_t len)
{
  ...
  LowStar_RVector_alloc____uint8_t__uint32_t(rg, vec, len);
  return vec;
}
...
{
  rg.r_free(rg.state, index__LowStar_Vector_vector_str__uint8_t_(rv, idx));
  if (idx != 0U)
    LowStar_RVector_free_elems__LowStar_Vector_vector_str__uint8_t__uint32_t(rg, rv, idx - 1U);
}
...
{
  if (!(rv.sz == 0U))
    LowStar_RVector_free_elems__LowStar_Vector_vector_str__uint8_t__uint32_t(rg, rv, rv.sz - 1U);
...
)
{
  ...
  if (j % 2U == 1U)
  {
    ...
    hash_fun(index___uint8_t_(lvhs, lvhs.sz - 2U), acc, acc);
    MerkleTree_Low_insert_(hsz, lv + 1U, j / 2U, hs, acc, hash_fun);
  }
}

bool MerkleTree_Low_mt_insert_pre(const MerkleTree_Low_merkle_tree *mt, uint8_t *v)
{
  KRML_MAYBE_UNUSED_VAR(v);
  MerkleTree_Low_merkle_tree mt1 = *(MerkleTree_Low_merkle_tree *)mt;
  return
    mt1.j
    < MerkleTree_Low_uint32_32_max
    && MerkleTree_Low_uint64_max - mt1.offset >= (uint64_t)(mt1.j + 1U);
}
...
void MerkleTree_Low_mt_insert(MerkleTree_Low_merkle_tree *mt, uint8_t *v)
{
  ...
  MerkleTree_Low_insert_(hsz1, 0U, mtv.j, hs, v, mtv.hash_fun);
  *mt
  =
    (
      (MerkleTree_Low_merkle_tree){
...
{
  return ((MerkleTree_Low_Datastructures_hash_vec){ .sz = 0U, .cap = vec.cap, .vs = vec.vs });
}
...
{
  cp(rg.state, v, index___uint8_t_(rv, i));
}
...
)
{
  if (!(j == 0U))
  {
    ...
    if (j % 2U == 0U)
      MerkleTree_Low_construct_rhs(hsz, lv + 1U, hs, rhs, i / 2U, j / 2U, acc, actd, hash_fun);
    else
    {
      if (actd)
      {
        assign_copy___uint8_t__uint32_t((
          ...
          lv,
          acc);
        hash_fun(index___uint8_t_(index__LowStar_Vector_vector_str__uint8_t_(hs, lv), j - 1U - ofs),
          acc,
          acc);
      }
      else
        hash_copy(hsz,
          index___uint8_t_(index__LowStar_Vector_vector_str__uint8_t_(hs, lv), j - 1U - ofs),
          acc);
      MerkleTree_Low_construct_rhs(hsz, lv + 1U, hs, rhs, i / 2U, j / 2U, acc, true, hash_fun);
    }
  }
}
...
{
  KRML_MAYBE_UNUSED_VAR(rt);
  MerkleTree_Low_merkle_tree *mt1 = (MerkleTree_Low_merkle_tree *)mt;
  MerkleTree_Low_merkle_tree mt2 = *mt1;
  KRML_HOST_IGNORE(mt2.hash_size);
  return true;
}
...
void MerkleTree_Low_mt_get_root(const MerkleTree_Low_merkle_tree *mt, uint8_t *rt)
{
  ...
  {
    MerkleTree_Low_construct_rhs(hash_size, 0U, hs, rhs, i, j, rt, false, hash_fun);
...
uint32_t MerkleTree_Low_mt_path_length(uint32_t lv, uint32_t k, uint32_t j, bool actd)
{
  ...
  {
    bool nactd = actd || j % 2U == 1U;
    return
      mt_path_length_step(k,
        j,
        actd)
      + MerkleTree_Low_mt_path_length(lv + 1U, k / 2U, j / 2U, nactd);
  }
}
...
)
{
  ...
  if (!(j == 0U))
  {
    ...
    {
      uint8_t
      *uu____0 = index___uint8_t_(index__LowStar_Vector_vector_str__uint8_t_(hs, lv), k - 1U - ofs1);
    ...
    if (j % 2U == 0U)
      ...
      ite = true;
    MerkleTree_Low_mt_get_path_(hsz, lv + 1U, hs, rhs, i / 2U, j / 2U, k / 2U, p, ite);
  }
}
...
)
{
  ...
  MerkleTree_Low_Datastructures_hash_vv hs = mtv.hs;
  MerkleTree_Low_Datastructures_hash_vec rhs = mtv.rhs;
  uint8_t *ih = index___uint8_t_(index__LowStar_Vector_vector_str__uint8_t_(hs, 0U), idx1 - ofs);
  MerkleTree_Low_path pth = *p;
  MerkleTree_Low_Datastructures_hash_vec pv = pth.hashes;
  MerkleTree_Low_Datastructures_hash_vec ipv = insert___uint8_t_(pv, ih);
  *p = ((MerkleTree_Low_path){ .hash_size = hsz1, .hashes = ipv });
  MerkleTree_Low_mt_get_path_(mtv.hash_size, 0U, hs, rhs, i, j, idx1, p, false);
  return j;
}
...
)
{
  ...
  if (!(oi == opi))
  {
    ...
    if (ofs >= hvec.sz)
      frv = ((MerkleTree_Low_Datastructures_hash_vec){ .sz = 0U, .cap = hvec.cap, .vs = hvec.vs });
    else if (ofs == 0U)
      frv = hvec;
    else
    {
      uint32_t n_shifted = hvec.sz - ofs;
      for (uint32_t i = 0U; i < n_shifted; i++)
        hvec.vs[0U + i] = hvec.vs[ofs + i];
      frv =
        (
          (MerkleTree_Low_Datastructures_hash_vec){
            .sz = n_shifted,
    ...
    MerkleTree_Low_mt_flush_to_(hsz, lv + 1U, hs, pi / 2U, i / 2U);
  }
}
...
void MerkleTree_Low_mt_flush_to(MerkleTree_Low_merkle_tree *mt, uint64_t idx)
{
  ...
  KRML_HOST_IGNORE(mtv.j);
  uint32_t hsz = mtv.hash_size;
  uint32_t idx1 = (uint32_t)(idx - offset);
  MerkleTree_Low_Datastructures_hash_vv hs = mtv.hs;
  MerkleTree_Low_mt_flush_to_(hsz, 0U, hs, mtv.i, idx1);
  *mt
  =
    (
      (MerkleTree_Low_merkle_tree){
...
)
{
  rg.r_free(rg.state, index___uint8_t_(rv, idx));
  if (idx + 1U < rv.sz)
    LowStar_RVector_free_elems_from___uint8_t__uint32_t(rg, rv, idx + 1U);
}
...
)
{
  ...
  {
    LowStar_RVector_free_elems_from___uint8_t__uint32_t(rg, rv, new_size);
...
)
{
  if (!(lv >= hs.sz))
  {
    MerkleTree_Low_Datastructures_hash_vec
    hvec = index__LowStar_Vector_vector_str__uint8_t_(hs, lv);
    KRML_HOST_IGNORE(j - MerkleTree_Low_offset_of(i));
    ...
    if (lv + 1U < hs.sz)
      MerkleTree_Low_mt_retract_to_(hsz, hs, lv + 1U, i / 2U, s / 2U, j / 2U);
  }
}
...
void MerkleTree_Low_mt_retract_to(MerkleTree_Low_merkle_tree *mt, uint64_t r)
{
  ...
  MerkleTree_Low_Datastructures_hash_vv hs = mtv.hs;
  MerkleTree_Low_mt_retract_to_(mtv.hash_size, hs, 0U, mtv.i, r1 + 1U, mtv.j);
  *mt
  =
    (
      (MerkleTree_Low_merkle_tree){
...
)
{
  ...
  if (!(j == 0U))
  {
    ...
    if (k % 2U == 0U)
      if (j == k || (j == k + 1U && !actd))
        MerkleTree_Low_mt_verify_(hsz, k / 2U, j / 2U, p, ppos, acc, nactd, hash_fun);
      else
      {
        MerkleTree_Low_path ncpd = *ncp;
        uint8_t *phash = index___uint8_t_(ncpd.hashes, ppos);
        hash_fun(acc, phash, acc);
        MerkleTree_Low_mt_verify_(hsz, k / 2U, j / 2U, p, ppos + 1U, acc, nactd, hash_fun);
      }
    else
    {
      MerkleTree_Low_path ncpd = *ncp;
  ...
  return
    ...
    &&
      ...
      ==
        1U
        +
          MerkleTree_Low_mt_path_length(0U,
...
)
{
  ...
  uint8_t *first = index___uint8_t_(pth.hashes, 0U);
  hash_copy(hash_size, first, ih);
  MerkleTree_Low_mt_verify_(hash_size, k1, j1, p, 1U, ih, false, mtv.hash_fun);
  uint8_t res = 255U;
  for (uint32_t i = 0U; i < hash_size; i++)
  {
...
serialize_bool(bool ok, bool x, uint8_t *buf, uint32_t sz, uint32_t pos)
{
  if (!ok || pos >= sz)
    return ((K___bool_uint32_t){ .fst = false, .snd = 0U });
  else
  {
    uint8_t ite;
    if (x)
      ...
      ite = 0U;
    buf[pos] = ite;
    return ((K___bool_uint32_t){ .fst = true, .snd = pos + 1U });
  }
}
...
{
  K___bool_uint32_t scrut = serialize_uint8_t(ok, (uint8_t)((uint32_t)x >> 8U), buf, sz, pos);
  bool ok1 = scrut.fst;
...
{
  K___bool_uint32_t scrut = serialize_uint16_t(ok, (uint16_t)(x >> 16U), buf, sz, pos);
  bool ok1 = scrut.fst;
  uint32_t pos1 = scrut.snd;
...
{
  K___bool_uint32_t scrut = serialize_uint32_t(ok, (uint32_t)(x >> 32U), buf, sz, pos);
  bool ok1 = scrut.fst;
  uint32_t pos1 = scrut.snd;
...
)
{
  ...
  {
    uint8_t b = x[i];
    K___bool_uint32_t scrut = serialize_uint8_t(ok, b, buf, sz, pos);
    bool ok1 = scrut.fst;
    uint32_t pos1 = scrut.snd;
    uint32_t j = i + 1U;
    if (j < hash_size)
      return MerkleTree_Low_Serialization_serialize_hash_i(hash_size, ok1, x, buf, sz, pos1, j);
    else
...
)
{
  if (!ok || pos >= sz)
    ...
    return MerkleTree_Low_Serialization_serialize_hash_i(hash_size, ok, x, buf, sz, pos, 0U);
}
...
)
{
  ...
  else
  {
    ...
    K___bool_uint32_t scrut = serialize_hash(hash_size, ok, vi, buf, sz, pos);
    bool ok1 = scrut.fst;
    uint32_t pos1 = scrut.snd;
    uint32_t j = i + 1U;
    if (j < x.sz)
      return MerkleTree_Low_Serialization_serialize_hash_vec_i(hash_size, ok1, x, buf, sz, pos1, j);
    else
      return ((K___bool_uint32_t){ .fst = ok1, .snd = pos1 });
  }
}
...
)
{
  ...
  {
    K___bool_uint32_t scrut = serialize_uint32_t(ok, x.sz, buf, sz, pos);
    bool ok1 = scrut.fst;
    uint32_t pos1 = scrut.snd;
    if (ok1 && x.sz > 0U)
      return
        MerkleTree_Low_Serialization_serialize_hash_vec_i(hash_size,
          ok1,
          x,
          buf,
          sz,
...
)
{
  ...
  else
  {
    ...
    uint64_t r;
    if (MerkleTree_Low_uint64_max - vs_hs >= 4ULL)
      r = vs_hs + 4ULL;
    else
      r = MerkleTree_Low_uint64_max;
    uint64_t rest = MerkleTree_Low_Serialization_hash_vv_bytes_i(hash_size, vv, i + 1U);
...
)
{
  ...
  else
  {
    ...
    K___bool_uint32_t scrut = serialize_hash_vec(hash_size, ok, vi, buf, sz, pos);
    bool ok1 = scrut.fst;
    uint32_t pos1 = scrut.snd;
    uint32_t j = i + 1U;
    if (j < x.sz)
      return MerkleTree_Low_Serialization_serialize_hash_vv_i(hash_size, ok1, x, buf, sz, pos1, j);
    else
      return ((K___bool_uint32_t){ .fst = ok1, .snd = pos1 });
  }
}
...
)
{
  ...
  else
  {
    ...
    uint32_t pos1 = scrut.snd;
    if (x.sz > 0U)
      return MerkleTree_Low_Serialization_serialize_hash_vv_i(hash_size, ok1, x, buf, sz, pos1, 0U);
    else
      return ((K___bool_uint32_t){ .fst = ok1, .snd = pos1 });
  }
}
...
deserialize_bool(bool ok, const uint8_t *buf, uint32_t sz, uint32_t pos)
{
  ...
  else
  {
    ...
    return ((__bool_uint32_t_bool){ .fst = true, .snd = pos + 1U, .thd = sw });
  }
}
...
{
  if (!ok || pos >= sz)
    return ((__bool_uint32_t_uint8_t){ .fst = false, .snd = pos, .thd = 0U });
  else
    return ((__bool_uint32_t_uint8_t){ .fst = true, .snd = pos + 1U, .thd = buf[pos] });
}
...
{
  if (!ok || pos >= sz)
    return ((__bool_uint32_t_uint16_t){ .fst = false, .snd = pos, .thd = 0U });
  else
  {
    ...
    return
      (
        (__bool_uint32_t_uint16_t){
          .fst = ok2,
          .snd = pos2,
          .thd = ((uint32_t)(uint16_t)b0 << 8U) + (uint32_t)(uint16_t)b1
        }
      );
  }
}
...
{
  if (!ok || pos >= sz)
    return ((__bool_uint32_t_uint32_t){ .fst = false, .snd = pos, .thd = 0U });
  else
  {
    ...
    return
      (
        (__bool_uint32_t_uint32_t){
          .fst = ok2,
          .snd = pos2,
          .thd = ((uint32_t)b0 << 16U) + (uint32_t)b1
        }
      );
  }
}
...
{
  if (!ok || pos >= sz)
    return ((__bool_uint32_t_uint64_t){ .fst = false, .snd = pos, .thd = 0ULL });
  else
  {
    ...
    return
      (
        (__bool_uint32_t_uint64_t){
          .fst = ok2,
          .snd = pos2,
          .thd = ((uint64_t)b0 << 32U) + (uint64_t)b1
        }
      );
  }
}
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (j < res.sz)
        return
          MerkleTree_Low_Serialization_deserialize_hash_vec_i(hash_size,
            ok1,
            buf,
            sz,
            pos1,
  ...
  else
  {
    ...
    else
    {
      ...
      if (j == res.sz)
        ...
        return
          MerkleTree_Low_Serialization_deserialize_hash_vv_i(hash_size,
            ok1,
            buf,
            sz,
            pos1,
...
uint64_t MerkleTree_Low_Serialization_mt_serialize_size(const MerkleTree_Low_merkle_tree *mt)
{
  ...
  MerkleTree_Low_Datastructures_hash_vv hs = mtv.hs;
  KRML_HOST_IGNORE(mtv.rhs_ok);
  MerkleTree_Low_Datastructures_hash_vec rhs = mtv.rhs;
  uint64_t hs_sz = MerkleTree_Low_Serialization_hash_vv_bytes_i(mtv.hash_size, hs, 0U);
  if (hs_sz < 4294967295ULL)
  {
    ...
    return 21ULL + hs_sz + 1ULL + ite + (uint64_t)mtv.hash_size;
  }
  else
    return MerkleTree_Low_uint64_max;
}
...
)
{
  ...
  MerkleTree_Low_merkle_tree mtv = *mt1;
  K___bool_uint32_t scrut = serialize_uint8_t(true, 1U, output, sz1, 0U);
  bool ok = scrut.fst;
  uint32_t pos = scrut.snd;
  K___bool_uint32_t scrut0 = serialize_uint32_t(ok, mtv.hash_size, output, sz1, pos);
  bool ok1 = scrut0.fst;
  uint32_t pos1 = scrut0.snd;
  ...
  K___bool_uint32_t scrut4 = serialize_hash_vv(mtv.hash_size, ok4, mtv.hs, output, sz1, pos4);
  bool ok5 = scrut4.fst;
  uint32_t pos5 = scrut4.snd;
  K___bool_uint32_t scrut5 = serialize_bool(ok5, mtv.rhs_ok, output, sz1, pos5);
  bool ok6 = scrut5.fst;
  uint32_t pos6 = scrut5.snd;
  K___bool_uint32_t scrut6 = serialize_hash_vec(mtv.hash_size, ok6, mtv.rhs, output, sz1, pos6);
  bool ok7 = scrut6.fst;
  uint32_t pos7 = scrut6.snd;
  K___bool_uint32_t scrut7 = serialize_hash(mtv.hash_size, ok7, mtv.mroot, output, sz1, pos7);
  bool ok8 = scrut7.fst;
...
{
  uint32_t sz1 = (uint32_t)sz;
  __bool_uint32_t_uint8_t scrut0 = deserialize_uint8_t(true, input, sz1, 0U);
  bool ok = scrut0.fst;
  uint32_t pos = scrut0.snd;
  uint8_t format_version = scrut0.thd;
  bool ok1 = ok && format_version == 1U;
  ...
  {
    __bool_uint32_t_uint64_t scrut0 = deserialize_offset_t(ok2, input, sz1, pos1);
    bool ok3 = scrut0.fst;
    uint32_t pos2 = scrut0.snd;
    uint64_t offset = scrut0.thd;
    __bool_uint32_t_uint32_t scrut1 = deserialize_index_t(ok3, input, sz1, pos2);
    bool ok4 = scrut1.fst;
    uint32_t pos3 = scrut1.snd;
    uint32_t i = scrut1.thd;
    __bool_uint32_t_uint32_t scrut2 = deserialize_index_t(ok4, input, sz1, pos3);
    bool ok5 = scrut2.fst;
    uint32_t pos4 = scrut2.snd;
    uint32_t j = scrut2.thd;
    __bool_uint32_t_MerkleTree_Low_Datastructures_hash_vv
    scrut3 = deserialize_hash_vv(hsz, ok5, input, sz1, pos4);
    bool ok6 = scrut3.fst;
    uint32_t pos5 = scrut3.snd;
    MerkleTree_Low_Datastructures_hash_vv hs = scrut3.thd;
    __bool_uint32_t_bool scrut4 = deserialize_bool(ok6, input, sz1, pos5);
    bool ok7 = scrut4.fst;
    uint32_t pos6 = scrut4.snd;
    bool rhs_ok = scrut4.thd;
    __bool_uint32_t_MerkleTree_Low_Datastructures_hash_vec
    scrut5 = deserialize_hash_vec(hsz, ok7, input, sz1, pos6);
    bool ok8 = scrut5.fst;
    uint32_t pos7 = scrut5.snd;
    MerkleTree_Low_Datastructures_hash_vec rhs = scrut5.thd;
...
)
{
  ...
  uint32_t sz1 = (uint32_t)sz;
  MerkleTree_Low_path *ncp = (MerkleTree_Low_path *)p;
  K___bool_uint32_t scrut = serialize_uint32_t(true, hsz1, output, sz1, 0U);
  bool ok = scrut.fst;
  uint32_t pos = scrut.snd;
  MerkleTree_Low_path ncpd = *ncp;
  K___bool_uint32_t scrut0 = serialize_hash_vec(hsz1, ok, ncpd.hashes, output, sz1, pos);
  bool ok1 = scrut0.fst;
  uint32_t pos1 = scrut0.snd;
  if (ok1)
...
{
  uint32_t sz1 = (uint32_t)sz;
  __bool_uint32_t_uint32_t scrut0 = deserialize_uint32_t(true, input, sz1, 0U);
  bool ok = scrut0.fst;
  uint32_t pos = scrut0.snd;
