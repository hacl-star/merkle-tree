...
int main(int argc, char *argv[]) {
  ...
  for (uint64_t k = 0; k < num_elts; k++) {
    ...
    printf("Verification with k=%"PRIu64" sz=%d: %d\n", k, sz, verified);
  ...
  printf("Flushed tree to [%"PRIu64"%"PRIu64"\n", flush_to, num_elts);
  ...
  for (uint64_t k = flush_to; k < num_elts; k++) {
    ...
    printf("Verification (after flushing) with k(%"PRIu64", j(%d): %d\n", k, j, verified);
...

  {
    ...
    for (uint64_t k = flush_to; k < num_elts; k++) {
      ...
      printf("Verification with k(%"PRIu64", j(%d): %d, deserialized (sz=%d): %d\n", k, j, verified, spsz, dverified);
  ...
  printf("Retracted tree to [%"PRIu64"%"PRIu64"\n", flush_to, retract_to);
  ...
  for (uint64_t k = flush_to; k <= retract_to; k++) {
    ...
    bool verified = mt_verify(mt, k, j, cur_path, root);
    printf("Verification with k(%"PRIu64", j(%d): %d\n", k, j, verified);
  ...
  printf("Flushed tree to [%"PRIu64"%"PRIu64"\n", flush_to, retract_to);
  {
    ...
    uint32_t j = mt_get_path(mt, k, cur_path, root);
    bool verified = mt_verify(mt, k, j, cur_path, root);
    printf("Final verification with k(%"PRIu64", j(%d): %d\n", k, j, verified);
    mt_free_path(cur_path);
  }
